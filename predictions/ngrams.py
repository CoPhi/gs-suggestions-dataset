...
from metrics import sentence_tokenizer, FALLBACK_LOSS, MIN_BEAM_SIZE, MAX_BEAM_SIZE
from config.settings import K_PRED, N, LM_TYPE, LAMBDA, ALPHA, BETA, DELTA
...
def get_sorted_filtered_words(df, head, tail, len_lacuna):
    ...
    if head and tail:
        return sort_and_filter(
            df,
            lambda w: (len(w) == (head_len + tail_len + len_lacuna))
            and (
                (w.startswith(head) and w.endswith(tail))
                or (w.startswith(head))
                or w.endswith(tail)
            ),
...
def get_best_candidates_from_beam(
    ...
    if mod == "acc":
        ...
        sorted_candidates = sorted(
            [candidate[0] for candidate in beam],
            key=lambda x: cached_edit_distance(" ".join(x), gold_label),
        )
