...
def test_umap_cohesion_comparison():
    ...
    if not valid_cases:
        raise ValueError(
            "Nessun caso valido trovato nel test set con lacuna a parola intera e 1 <= gap_length <= 6"
        )
    ...
    print(
        f"Gold Label attesa: '{gold_text}' (lunghezza lacuna stimata: {case.gap_length} caratteri)\n"
    )

    results = {}

    for label, checkpoint in [
        ("Pre-FT", pre_ft_checkpoint),
        ("Post-FT", post_ft_checkpoint),
    ]:
        ...
        if candidate_embeddings:
            ...
            pooled_gold = (
                gold_embedding.mean(dim=0).unsqueeze(0)
                if gold_embedding.dim() != 1
                else gold_embedding.unsqueeze(0)
            )
            ...
            print(
                f"Plausibilità (sim. coseno media candidati vs Gold): {mean_gold_sim:.4f}"
            )
            print(
    ...
    if "Pre-FT" in results and "Post-FT" in results:
        ...
        print(
            f"Coesione Intra-Cluster: Pre-FT = {pre['cohesion']:.4f} | Post-FT = {post['cohesion']:.4f}"
        )
        if pre["cohesion"] is not None and post["cohesion"] is not None:
            diff_cohesion = post["cohesion"] - pre["cohesion"]
            cohesion_status = (
                "AUMENTATA (Migliore)" if diff_cohesion > 0 else "DIMINUITA"
            )
            print(f"  -> La coesione è {cohesion_status} di {abs(diff_cohesion):.4f}")

        print(
            f"Plausibilità Media vs Gold: Pre-FT = {pre['mean_gold_similarity']:.4f} | Post-FT = {post['mean_gold_similarity']:.4f}"
        )
        ...
        print(
            "1. Coesione più alta post-FT indica che il modello fine-tuned restringe lo spazio delle ipotesi"
        )
        print(
            "   generando suggerimenti più vicini semanticamente (più coerenti nel contesto)."
        )
        print(
            "2. Plausibilità media più alta post-FT indica che i suggerimenti generati dal modello fine-tuned"
        )
        print(
            "   sono semanticamente e contestualmente più affini alla parola reale attesa (Gold Label)."
        )
        print(
            "3. UMAP Plotting: Visivamente, nel grafico post-FT, la stella rossa (Gold) dovrebbe trovarsi"
        )
        print(
            "   più integrata all'interno del cluster blu (Suggerimenti) rispetto al grafico pre-FT."
        )
    else:
        print(
            "Impossibile effettuare il confronto (dati mancanti per una delle due fasi)."
        )
